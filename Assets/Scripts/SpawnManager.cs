...
public class SpawnManager : MonoBehaviour
{
    ...
    void Update()
    {
        enemyCount = FindObjectsOfType<Enemy>().Length; //count how many enemies exist atm

        if (enemyCount == 0)
        {
            waveNumber++;
            SpawnEnemyWave(waveNumber);
            Instantiate(powerupPrefab, GenerateSpawnPosition(), powerupPrefab.transform.rotation); //get a powerup every wave
        }
    }
